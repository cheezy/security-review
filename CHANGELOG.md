...
The Step 6 exit-code table documented exit `2` for an "agent dispatch failure," but the three agent-response-failure branches — Step 4a (diff-mode dispatch), Step 4b (full-mode batch), and Step 4.5 (RCI pass) — only printed a one-line error and "stopped," never running `exit 2`. A `--fail-on` CI gate therefore observed an unspecified exit code (not the documented `2`) when the reviewer returned unparseable JSON. **Resolution (Option A):** each of the three branches now runs a final `exit 2` via Bash after printing its error, mirroring the Step 1 misuse pattern used by every other exit-2 case (`--fail-on`, `--base`, `--considerations`, `--sarif`/`--json` conflict). The exit-code table's `0`/`1`/`2` meanings are unchanged; only the procedure was reconciled to the contract. This is the source-of-truth fix; the four runtime ports (OpenCode, Gemini, Copilot, Codex) apply the identical Option A resolution.

## [2.5.0] - 2026-07-22

### Added — `considerations` mode verifies a task's security_considerations were actually mitigated (G366: W1857–W1860)
...
The capability is exposed both programmatically (declare `considerations` mode in the agent prompt) and via a new `--considerations <source>` slash-command flag, where `<source>` is a file (one consideration per line) or an inline consideration, resolved and read as untrusted data — never shell-executed. The flag renders a `## Security considerations` block grouping each consideration with its verdict and evidence. `consideration_verdicts` is emitted **only** in considerations mode, so output stays byte-identical for callers that don't opt in — exactly like the existing `--maestro`/`--patches` opt-ins. `--considerations` is diff-based and is ignored under `--full`.

Eval coverage adds a considerations lane to the TAP runner: paired positive-control (an unmitigated consideration with a backing finding) and negative-control (all mitigated, no spurious finding) fixtures under `test/fixtures/considerations/`, a `→ CONSIDERATIONS …` EXPECTED.md grammar, and a `check_fixtures.sh` parity exclusion for the `.considerations` sidecars — bringing the suite to 72 fixtures. `consideration_verdicts` is non-deterministic agent output, so it is asserted by the eval runner rather than golden-compared.

This is the Claude Code plugin surface of the capability; the five runtime ports and the Stride-orchestrator/kanban-app integration are separate, out-of-scope work.
...
- **Android / Kotlin framework rule pack.** New pack at `agents/security-reviewer.md` at the top of the framework-pack section (alphabetical: A before D). Activation: `.kt` / `.java` / `.gradle` AND import of `android.*` / `androidx.*` / `kotlinx.*` / `dagger.hilt.android.*` / `com.google.android.*`, OR file named `AndroidManifest.xml`. Six rules: (1) exported `<activity>` / `<service>` / `<receiver>` without signature-level permission (authorization, CWE-927); (2) WebView `setJavaScriptEnabled(true)` + `addJavascriptInterface` on a dynamic-URL load (xss_or_code_exec, critical, CWE-79); (3) `usesCleartextTraffic="true"` without `networkSecurityConfig` (insecure_config, CWE-319); (4) `SharedPreferences` storing sensitive data instead of `EncryptedSharedPreferences` (data_exposure, CWE-922) — non-trigger for non-sensitive prefs; (5) `SQLiteDatabase.rawQuery` / `execSQL` with string concatenation (injection, critical, CWE-89) — non-trigger for `selectionArgs` / `bindArgs` parameterized form; (6) trust-all `HostnameVerifier` / empty `X509TrustManager.checkServerTrusted` (crypto, critical, CWE-295). Comes with `android_exported_no_permission.xml`, `android_webview_js_interface.kt`, `android_cleartext_traffic.xml`, `android_sharedprefs_token.kt`, `android_sqlite_injection.kt`, `android_hostname_verifier_trust_all.kt`. Prior art: MobSF, Android Lint security rules, QARK — pattern-based; the semantic-analysis differentiator is intent-flow tracking (exported component → intent extras → action dispatch) and AndroidManifest cross-referencing of `android:permission` against the declared protectionLevel.
- **iOS / Swift framework rule pack.** New pack at `agents/security-reviewer.md` between the Express and Phoenix packs (alphabetical: e < i < p). Activation: `.swift` / `.m` / `.mm` / `.h` extensions AND import of `UIKit` / `SwiftUI` / `Foundation` / `WebKit` / `Security` / `CommonCrypto` / `CryptoKit` / `Network`, OR a file named `Info.plist` (XML; the ATS rule activates on its content). Five rules: (1) sensitive data in `UserDefaults` (data_exposure, CWE-922) — non-trigger for non-sensitive prefs; (2) `WKWebView` with `allowFileAccessFromFileURLs` or unverified-origin JS bridge (xss_or_code_exec, CWE-79); (3) ATS `NSAllowsArbitraryLoads = true` without `NSExceptionDomains` (insecure_config, CWE-319); (4) `URLSession` delegate trusting any server certificate (crypto, CWE-295, critical) — non-trigger for `SecTrustEvaluateWithError` / pinned-cert evaluation; (5) deep-link handler dispatching state-changing actions without `LAContext().evaluatePolicy(...)` (authorization, CWE-939) — non-trigger for read-only navigation. Comes with `ios_userdefaults_password.swift`, `ios_wkwebview_file_access.swift`, `ios_ats_arbitrary_loads.plist`, `ios_url_session_trust_all.swift`, `ios_deep_link_no_auth.swift`. Prior art: MobSF, OWASP MASVS, Xcode's static analyzer — pattern-based; the semantic-analysis differentiator is trust-boundary mapping (URL scheme → handler → action) and Info.plist cross-referencing.
...
[1.2.2]: https://github.com/cheezy/stride-security-review/releases/tag/v1.2.2
[1.2.1]: https://github.com/cheezy/stride-security-review/releases/tag/v1.2.1
